                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    switch (command)
                    {
                        case "get":
                            ...
                            if (value != null)
                                await SendAnswerToClient(socket, JsonSerializer.SerializeToUtf8Bytes(value), token);
                            else
                                await SendAnswerToClient(socket, NIL, token);
                            break;
                        case "set":
                            _simpleStore.Set(Encoding.UTF8.GetString(result.Key), JsonSerializer.Deserialize<UserProfile>(result.Value));
