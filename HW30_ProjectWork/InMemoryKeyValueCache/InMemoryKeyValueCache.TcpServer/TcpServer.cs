...
using InMemoryKeyValueCache.Common.Parsers;
...
namespace InMemoryKeyValueCache.TcpServer
{
    public class TcpServer : IDisposable
    {
        ...
        public TcpServer(SimpleStore simpleStore)
        {
            ...
            _semaphoreSlim = new SemaphoreSlim(Constraints.MaxClientSemaphore, Constraints.MaxClientSemaphore);
        }
        public void Dispose()
        {
            Dispose(true);
                    ...
                    switch (command)
                    {
                        case "get":
                            ...
                            if (value != null)
                            {
                                ...
                                await SendAnswerToClient(socket, CommonBytesData.Nil, token);
                            break;
                        case "set":
                            ...
                            await SendAnswerToClient(socket, CommonBytesData.Ok, token);
                            break;
                        case "delete":
                            key = Encoding.UTF8.GetString(result.Key);
                            activity?.SetTag("Command.Key", key);
                            ...
                            break;
                        default:
                            await SendAnswerToClient(socket, CommonBytesData.Error, token);
                            break;
                    }
